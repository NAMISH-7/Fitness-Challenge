...
export default function EditEventPage() {
  ...
  const [eventData, setEventData] = useState<typeof events[0] | null>(null);
  ...
  useEffect(() => {
    ...
    if (event) {
      ...
      setEventData(event);
      setIsVirtual(event.type === 'virtual');
  ...
  if (!eventData) return <div className="p-8 ml-64">Loading...</div>;
        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            ...
            <p className="text-text-secondary-light dark:text-text-secondary-dark mt-1">
              Update details for {eventData.title}.
            </p>
          </motion.div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-1">
                    ...
                    <input 
                      type="datetime-local" 
                      defaultValue={eventData.date ? (eventData.date.includes('T') ? eventData.date : `${eventData.date}T09:00`) : ""}
                      required
                      className="w-full px-4 py-2.5 rounded-xl border border-border-light dark:border-border-dark bg-surface-light dark:bg-surface-dark text-text-primary-light dark:text-text-primary-dark focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all"
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <input 
                      type="datetime-local" 
                      defaultValue={eventData.endDate ? (eventData.endDate.includes('T') ? eventData.endDate : `${eventData.endDate}T17:00`) : ""}
                      className="w-full px-4 py-2.5 rounded-xl border border-border-light dark:border-border-dark bg-surface-light dark:bg-surface-dark text-text-primary-light dark:text-text-primary-dark focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all"
                    />
                  </div>
                </div>
      ...
      {isSuccess && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          ...
          >
            ...
            <h3 className="text-xl font-bold text-text-primary-light dark:text-text-primary-dark mb-2">Changes Saved!</h3>
            <p className="text-text-secondary-light dark:text-text-secondary-dark text-sm">
              The event details have been successfully updated. Redirecting you back to the events dashboard...
            </p>
          </motion.div>
        </div>
      )}
